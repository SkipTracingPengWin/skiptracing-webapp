...
export default function ActionModal({
    ...
    useEffect(() => {
        if (isOpen) {
            if (borrowers.length === 0) fetchBorrowers();
            if (agents.length === 0) fetchAgents();
        }
    }, [isOpen, borrowers.length, agents.length, fetchBorrowers, fetchAgents]);
    ...
    const handleSubmit = (e: any) => {
        ...
        const submittedData = {
            id: Date.now(), // Fallback ID, but backend should generate
            type: title,
            actionId: actionId,
            // Map actionId to the string key expected by page.tsx iconMap
            icon: actionId === "sms" ? "MessageSquare" :
                actionId === "manual_call" ? "Phone" :
                    actionId === "ivr" ? "Video" :
                        actionId === "visit" ? "MapPin" :
                            actionId === "legal" ? "FileText" :
                                actionId === "whatsapp" ? "MessageCircle" : "MessageSquare",
            borrowerId: formData.borrowerId, // Ensure these are captured
